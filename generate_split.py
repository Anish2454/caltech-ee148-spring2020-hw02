...
Your code below.
'''
file_names_train = np.random.choice(file_names, int(0.85*len(file_names)),
                                    replace=False)
file_names_test = list(set(file_names) - set(file_names_train))
...
if split_test:
    ...
    for filename in file_names:
        if filename in file_names_train:
